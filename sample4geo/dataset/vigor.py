...
class VigorDatasetTrain(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.ext == 'pt':
            ...
            query_path = self.idx2ground_path[idx_ground]
            query_path = query_path.replace('.jpg', '.pt') if '.jpg' in query_path else query_path.replace('.png', '.pt')
            with open(query_path, 'rb') as _f1:
                    query_img  = pickle.load(_f1)
            
            # load reference -> satellite image
            reference_path = self.idx2sat_path[idx_sat]
            reference_path = reference_path.replace('.jpg', '.pt') if '.jpg' in reference_path else reference_path.replace('.png', '.pt')
...
class VigorDatasetEval(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.ext == 'pt':
            img_path = img_path.replace('.jpg', '.pt') if '.jpg' in img_path else img_path.replace('.png', '.pt')
            with open(img_path, 'rb') as _f1:
                    img  = pickle.load(_f1)
        else:
