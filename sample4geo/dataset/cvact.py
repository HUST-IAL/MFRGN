...
class CVACTDatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        for idx in train_ids.squeeze():
            ...
            grd_path = f'ANU_data_small/streetview/{idx}_grdView.{self.ext}'
            sat_path = f'ANU_data_small/satview_polish/{idx}_satView_polish.{self.ext}'
    ...
    def __getitem__(self, index):
        ...
        query_path = f'{self.data_folder}/ANU_data_small/streetview/{idx}_grdView.{self.ext}'
        if self.is_crop:
            query_path = query_path.replace("streetview", "streetview_processed")
            query_path = query_path.replace("grdView.jpg", "grdView.png")
        if self.ext == 'pt':
            with open(query_path, 'rb') as _f1:
                    query_img  = pickle.load(_f1)
        else:
            query_img = cv2.imread(query_path)
            query_img = cv2.cvtColor(query_img, cv2.COLOR_BGR2RGB)
        
        # load reference -> satellite image
        reference_path = f'{self.data_folder}/ANU_data_small/satview_polish/{idx}_satView_polish.{self.ext}'
        if self.is_polar:
            reference_path = f'{self.data_folder}/ANU_data_small/polarmap/{idx}_satView_polish.{self.ext}'

        if self.ext == 'pt':
            with open(reference_path, 'rb') as _f1:
                    reference_img  = pickle.load(_f1)
        else:
...
class CVACTDatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        for idx in ids.squeeze():
            ...
            grd_path = f'{self.data_folder}/ANU_data_small/streetview/{idx}_grdView.{self.ext}'
            sat_path = f'{self.data_folder}/ANU_data_small/satview_polish/{idx}_satView_polish.{self.ext}'
    ...
    def __getitem__(self, index):
        ...
        if self.img_type == "reference":
            ...
            if self.is_polar:
                path = f'{self.data_folder}/ANU_data_small/polarmap/{idx}_satView_polish.png'

        elif self.img_type == "query":
            path = f'{self.data_folder}/ANU_data_small/streetview/{idx}_grdView.jpg'
            if self.is_crop:
                path = path.replace("streetview", "streetview_processed")
                path = path.replace("grdView.jpg", "grdView.png")

        if self.ext == 'pt':
            path = path.replace('.jpg', '.pt') if '.jpg' in path else path.replace('.png', '.pt')
            with open(path, 'rb') as _f1:
                    img  = pickle.load(_f1)
        else:
            img = cv2.imread(path)
...
class CVACTDatasetTest(Dataset):
    ...
    def __init__(self,
        ...
        files_sat = glob(f'{self.data_folder}/ANU_data_test_ori/satview_polish/*_satView_polish.{ext}')
        files_ground = glob(f'{self.data_folder}/ANU_data_test_ori/streetview/*_grdView.{ext}')
        
        sat_ids = []
        b = 1 if ext == 'pt' else 0
        for path in files_sat:
            idx = path.split("/")[-1][:-19+b]
            sat_ids.append(idx)
        
        ground_ids = []
        for path in files_ground:
            idx = path.split("/")[-1][:-12+b]
    ...
    def __getitem__(self, index):
        ...
        if self.img_type == "reference":
            path = f'{self.data_folder}/ANU_data_test_ori/satview_polish/{idx}_satView_polish.{self.ext}'
        else:
            path = f'{self.data_folder}/ANU_data_test_ori/streetview/{idx}_grdView.{self.ext}'
            if self.is_crop:
                path = path.replace("streetview", "streetview_processed")
                path = path.replace("grdView.jpg", "grdView.png")
