...
def evaluate(config,
    ...
    s1 = time.time()
    if is_dual:
        query_features, reference_features, query_labels, reference_labels = predict_dual(config, model, query_dataloader, reference_dataloader, is_autocast=is_autocast)
    else:
        reference_features, reference_labels = predict(config, model, reference_dataloader, is_autocast=is_autocast, input_id=2)
        query_features, query_labels = predict(config, model, query_dataloader, is_autocast=is_autocast, input_id=1)

    s2 = time.time()
    print('Extract Features time: ', (s2 - s1)/60, 'min')
...
def calc_sim(config,
    ...
    if is_dual:
        ...
        reference_features, reference_labels = predict(config, model, reference_dataloader, is_autocast=is_autocast)
        query_features, query_labels = predict(config, model, query_dataloader, is_autocast=is_autocast)
